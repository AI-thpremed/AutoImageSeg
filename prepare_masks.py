...
def prepare_masks_and_mapping(cfg: Dict[str, Any], log) -> None:
    """
    cfg :
        train_json, test_json, cache, results
    """
    ...
    label2id = {lbl: idx + 1 for idx, lbl in enumerate(sorted(label_set))}
    label2id['_background_'] = 0

    with open(mapping_file, 'w', encoding='utf-8') as f:
        json.dump(label2id, f, ensure_ascii=False, indent=2)
    log(f'[prepare] label -> id: {label2id}')

    # ---------- 生成 train masks ----------
    train_jsons = list(Path(train_json_dir).rglob('*.json'))
    train_mask_dir = os.path.join(cache_dir, 'train_masks')
    os.makedirs(train_mask_dir, exist_ok=True)

    for js in tqdm(train_jsons,
                   desc='[prepare] train masks',
                   file=TqdmToLog(log)):
        data = json.load(open(js, encoding='utf-8'))
        h, w = data['imageHeight'], data['imageWidth']
        mask = shapes_to_label_mask(str(js), label2id, policy_map, h, w)
        out_path = os.path.join(train_mask_dir, js.stem + '.png')
        cv2.imwrite(out_path, mask)

    # ---------- 生成 test masks ----------
    test_jsons = list(Path(test_json_dir).rglob('*.json'))
    test_mask_dir = os.path.join(cache_dir, 'test_masks')
    os.makedirs(test_mask_dir, exist_ok=True)

    for js in tqdm(test_jsons,
                   desc='[prepare] test masks',
                   file=TqdmToLog(log)):
        data = json.load(open(js, encoding='utf-8'))
        h, w = data['imageHeight'], data['imageWidth']
        mask = shapes_to_label_mask(str(js), label2id, policy_map, h, w)
        out_path = os.path.join(test_mask_dir, js.stem + '.png')
        cv2.imwrite(out_path, mask)

    log(f'[prepare] masks & mapping ready in {cache_dir}')
...
def prepare_masks_and_mapping_mask(cfg: Dict[str, Any], log) -> None:
    try:
        ...
        for mask_dir, target_dir in [(train_mask_dir, train_masks_dir),
                                     (test_mask_dir, test_masks_dir)]:
            files = sorted(os.listdir(mask_dir))
            for mask_file in tqdm(files,
                                  desc=f"Copy {os.path.basename(mask_dir)}",
                                  file=TqdmToLog(log)):
                mask_path = os.path.join(mask_dir, mask_file)
                dst_path = os.path.join(target_dir, mask_file)
                shutil.copy2(mask_path, dst_path)

                # for mask_dir, target_dir in [(train_mask_dir, train_masks_dir),
        #                              (test_mask_dir, test_masks_dir)]:
        #     files = sorted(os.listdir(mask_dir))  # 保证顺序可复现
        #     for mask_file in tqdm(files, desc=f"Copy {os.path.basename(mask_dir)}"):  # 进度条
        #         mask_path = os.path.join(mask_dir, mask_file)
        ...
        #         mask_path = os.path.join(mask_dir, mask_file)
                ...
                with Image.open(mask_path) as img:
                    ...
                    # ---------- always save as PNG ----------
                    png_name = os.path.splitext(mask_file)[0] + '.png'
                    target_path = os.path.join(target_dir, png_name)
        ...
        for label, id in label2id.items():
            ...
            for mask_dir in [train_masks_dir, test_masks_dir]:
                for png_file in os.listdir(mask_dir):
                    with Image.open(os.path.join(mask_dir, png_file)) as img:

...
def train_worker_mask(cfg: dict, log_q):
    ...
    project_dir = root_cache   / f'train_mask_project_{timestamp}'
    res_dir     = root_results / f'train_mask_project_{timestamp}'
    ...
    try:
        ...
        train_dl = DataLoader(train_ds, batch_size=config["training"]["dataloader"]["batch_size"], shuffle=config["training"]["dataloader"]["shuffle"],  num_workers=num_workers,pin_memory=True)
        val_dl   = DataLoader(val_ds,   batch_size=config["training"]["dataloader"]["batch_size"], shuffle=False, num_workers=num_workers,pin_memory=True)
        ...
        lr = config["training"].get("lr", 1e-4)  # 默认值为 1e-4
        optimizer = Adam(model.parameters(), lr=lr)

        evemetric = config["training"].get("evemetric", "iou")  # 默认值为 "iou"

        scheduler = CosineAnnealingLR(optimizer, T_max=cfg['epochs'])

        earlystop = config["training"].get("earlystop", 0)  # 默认值为 0
